...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initStandardSectionController(config) {
    ...
    function renderActionButtons() {
      ...
      } else if (state.assessment === 'sa1' && sectionParam === 'bilingual' && state.classId <= 5) {
        const sa1Btn = document.createElement('a');
        sa1Btn.className = 'action-btn action-qp';
        sa1Btn.style.maxWidth = '340px';
        sa1Btn.style.margin = '0 auto';
        sa1Btn.href = `resources.html?section=${sectionParam}&class=${state.classId}&assessment=sa1`;
        sa1Btn.innerHTML = `
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          View Complete SA-1 Packages
        `;
        buttonsContainer.appendChild(sa1Btn);
      } else {
        let qpHref = '';
        let akHref = '';
